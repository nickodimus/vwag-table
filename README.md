...
- **Import prebuilt maps** — bring in a DungeonDraft `.dtt` module or a Universal VTT file (`.uvtt` / `.dd2vtt` / `.df2vtt` — the interchange format exported by DungeonDraft, Dungeon Alchemist, Dungeon Fog, Mapforge, and others) and the map arrives with its grid auto-calibrated and its walls, doors, and lights already placed, so line-of-sight and dynamic light work immediately. Roll20 and Owlbear Rodeo maps are supported via their Universal VTT export.
