...
- **On-screen token step arrows** — eight pop-out direction arrows on a selected token for single-step movement, including diagonals, with live distance. Keyboard-free, so the GM can nudge a token by finger on the touch table.
